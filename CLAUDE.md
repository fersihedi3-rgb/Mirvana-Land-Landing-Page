...
`css/mirvana.css`, `css/fonts.css`, `js/mirvana.js`, `js/consent.js`, and locally hosted
...
The pages describe the form's real behaviour — WhatsApp handoff and the
`mirvana_leads` localStorage copy. **If `ENDPOINT` is ever set, section 4 of
the privacy policy and its "En bref" box stop being true** and must be
updated in the same change.

Company registration facts nobody in the repo knows (raison sociale, RC/ICE/IF,
siège social, capital, directeur de la publication, hébergeur) are not
published. Both pages say they are given on request, and the phone number is
the working contact channel. Filling them in later means editing the
`.legal-facts` list in each page and dropping that sentence — don't invent
values.

## SEO

Canonical host is `https://mirvana-land.com` and it is **hardcoded** in
`index.html`, both legal pages, `sitemap.xml` and `robots.txt`. A domain change
is a find-and-replace across those five files; there is no build step to
templatise it.

Per page: `<link rel="canonical">`, `og:*` with an **absolute** `og:image`
(relative ones silently break every link preview), `twitter:card`,
`theme-color`, the favicon set and the manifest. `index.html` also carries a
JSON-LD `@graph` of `Organization` + `WebSite` + `Product`/`AggregateOffer`.
It states only what the page states: no rating, no `offerCount` (that would
claim 62 villas are still available) and no geo coordinates.

Images are generated by ffmpeg from files already in the repo, not fetched:
`og-image.jpg` is a 1200×630 crop of `firepit.jpg`; `icon-512/192`,
`favicon-32` and `apple-touch-icon` come from the **complete** ML monogram
inside `logo-lock.png`. Do not regenerate icons from `logo-mono.png` — the
glyph is cropped off its canvas there and loses the M and the L.

## Analytics and consent

`js/consent.js` is the only code that loads anything from Google, and
**`GA_ID` is empty**, which is the cookie-free state: no tag, no banner, no
third-party request. Setting `GA_ID` to a `G-XXXXXXXXXX` is the whole switch.

The visitors are mostly in France, so the CNIL rules are implemented, not
approximated: Consent Mode v2 defaults are queued as denied before any tag can
exist; the banner's two answers are the same size and weight; Escape counts as
a refusal; a refusal is remembered for 182 days and re-asked once after that;
acceptance stands until withdrawn through "Gérer mes cookies". The lead event
carries only the qualifying answers (budget, projet, delai, financement) and
never the name, phone, e-mail or free-text message.

`data-consent-scope` keeps the privacy policy honest in both states: elements
marked with it appear only when `GA_ID` is set, and `data-consent-scope="off"`
only when it is empty — which is how section 8 says measurement is currently
inactive without anyone having to remember to edit it.
...
- **`heroSlideshow`** keeps later slides in `data-src` and only shows a slide
  ...
  pause control remains. A fourth gate, `blocked`, stops it while the consent
  banner covers that pause control; it is seeded from the `.consent-open`
  class rather than only from the `mirvana:consent` event, because both files
  are `defer` and `consent.js` runs first — the event has already fired by the
  time this module registers a listener.
